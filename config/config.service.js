...
class Config {
  checkingSettings() {
    try {
      let yandex = false;
      if (
        (process.env.VOICE_RESPONSE === 'true' && process.env.SPEECH_TO_TEXT_MODE === 'yandex') ||
        process.env.TEXT_TO_SPEECH_MODE === 'yandex'
      )
        yandex = true;
      ...
      settingFields.forEach((key, value) => {
        if (key === 'VOICE_RESPONSE') {
          ...
          if (voiceResponse !== 'true' && voiceResponse !== 'false')
            throw new Error(`'VOICE_RESPONSE': 'true' or 'false'`);
        }
        if (key === 'SPEECH_TO_TEXT_MODE') {
          const sttMode = this.get(key);
          if (sttMode !== 'openai' && sttMode !== 'yandex')
            throw new Error(`'SPEECH_TO_TEXT_MODE': 'openai' or 'yandex'`);
        }
        if (key === 'TEXT_TO_SPEECH_MODE') {
          const ttsMode = this.get(key);
          if (ttsMode !== 'google' && ttsMode !== 'yandex' && ttsMode !== 'any')
            throw new Error(`'TEXT_TO_SPEECH_MODE': 'google', 'yandex' or 'any'`);
        }
        ...
        if (key === 'SAVE_VOICE_HISTORY') {
          ...
          if (
            saveVoiceMode !== 'request' &&
            saveVoiceMode !== 'response' &&
            saveVoiceMode !== 'any' &&
            saveVoiceMode !== 'false'
          )
        ...
        if (key === 'VOICE_SPEED_NORMAL') {
          ...
          if (2 < +voiceSpeedNormal || +voiceSpeedNormal < 1)
            throw new Error(`'VOICE_SPEED_NORMAL': the value "1.0" - "2.0". Example: '1.5'`);
        }
        if (key === 'VOICE_SPEED_SLOW') {
          const voiceSpeedSlow = this.get('VOICE_SPEED_SLOW');
          if (2 < +voiceSpeedSlow || +voiceSpeedSlow < 1)
            throw new Error(`'VOICE_SPEED_SLOW': the value "1.0" - "2.0". Example: '1.3'`);
        }
        if (key === 'VOICE_SPEED_FAST') {
          const voiceSpeedFast = this.get('VOICE_SPEED_FAST');
          if (2 < +voiceSpeedFast || +voiceSpeedFast < 1)
            throw new Error(`'VOICE_SPEED_FAST': the value "1.0" - "2.0". Example: '1.8'`);
        }
  ...
  async checkingWhitelist() {
    try {
      const whitelist = await fsPromises
        .readFile(resolve(__dirname, '../config', 'whitelist.config.json'), 'utf8')
  ...
  checkingLoggerSettings() {
    try {
      const loggerFields = [
        ...
        'LOG_FILENAME',
        'LOG_FILENAME_INFO',
        'LOG_FILENAME_WARNING',
        'LOG_FILENAME_ERROR',
      ];
      loggerFields.forEach((key) => {
        if (key === 'LOG_MODE') {
          const logMode = this.get('LOG_MODE');
          if (logMode !== 'one_file' && logMode !== 'different_files' && logMode !== 'together')
            throw new Error(`'LOG_MODE': 'one_file', 'different_files' or 'together'`);
        }
        if (key === 'LOG_PATH') {
        ...
        if (key === 'LOG_FILENAME') {
          ...
          if (logFilename.length <= 0)
            throw new Error(`'LOG_FILENAME': the value cannot be empty. Example: 'common.log.txt'`);
        }
        if (key === 'LOG_FILENAME_INFO') {
          const logFilenameInfo = this.get('LOG_FILENAME_INFO');
          if (logFilenameInfo.length <= 0)
            throw new Error(`'LOG_FILENAME_INFO': the value cannot be empty. Example: 'info.log.txt'`);
        }
        if (key === 'LOG_FILENAME_WARNING') {
          const logFilenameWarn = this.get('LOG_FILENAME_WARNING');
          if (logFilenameWarn.length <= 0)
            throw new Error(`'LOG_FILENAME_WARNING': the value cannot be empty. Example: 'warnings.log.txt'`);
        }
        if (key === 'LOG_FILENAME_ERROR') {
          const logFilenameErr = this.get('LOG_FILENAME_ERROR');
          if (logFilenameErr.length <= 0)
            throw new Error(`'LOG_FILENAME_ERROR': the value cannot be empty. Example: 'errors.log.txt'`);
        }
      });
