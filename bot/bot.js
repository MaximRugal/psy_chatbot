...
class Bot {
  ...
  launch() {
    ...
    this.bot.launch({ webhook: { domain: webhookDomain, port: 5000 } });
  ...
  async setUserSettings(userId) {
    try {
      ...
      for (let settingsKey in settings) {
        if (settings[settingsKey] !== this.userSettings.get(settingsKey))
          this.userSettings.set(settingsKey, settings[settingsKey]);
      }
