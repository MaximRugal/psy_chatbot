...
import { psyConfig } from '../../config/roleConfigs.js';

class Commands {
  botCommands(telegramBot) {
    telegramBot.bot.telegram.setMyCommands([
      { command: 'start', description: 'Запустить чат' },
      { command: 'restart', description: 'Перезапустить и сбросить контекст' },
  ...
  start(telegramBot) {
    telegramBot.bot.command('start', async (ctx) => {
      ...
      ctx.session.messages.push({ role: 'system', content: psyConfig });
      await ctx.reply(
        `Приветствую, ${ctx.message.from.first_name}! Для общения со мной можно использовать голосовые или текстовые сообщения.`,
  ...
  restart(telegramBot) {
    telegramBot.bot.command('restart', async (ctx) => {
      ...
      ctx.session.messages.push({ role: 'system', content: psyConfig });
      await ctx.reply('Контекст очищен. Отправте мне новый голосовой или текстовый запрос', {
        reply_markup: { remove_keyboard: true },
      });
