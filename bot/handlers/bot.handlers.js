...
import { psyConfig, gregoConfig } from '../../config/roleConfigs.js';
    ...
    telegramBot.bot.on(message('voice'), async (ctx) => {
      try {
        ...
        if (!ctx.session) {
          ...
          ctx.session.messages.push({ role: openAi.roles.USER, content: psyConfig });
        }
        ...
        if (JSON.stringify(telegramBot.whitelist.users).includes(ctx.message.from.username)) {
          ...
          //   if (!ctx.session) {
          //   await openAi.chat({ role: openAi.roles.SYSTEM, content: psyConfig });
          //   }
          ...
          // Voice response
          //   if (telegramBot.userSettings.get('responseMode') !== 'text')
          //     await ctx.reply(code(`Ожидаем голосового ответа...`));
          ...
          await ctx.reply(
            code(`Извините, ${ctx.message.from.first_name}, у Вас нет разрешения на использование бота.`)
          );
          Logger.info(
            `User: ${ctx.message.from.first_name} id: ${ctx.message.from.id}`,
            'bot.handlers',
            '',
            'NO ACCESS',
            'x'
    ...
    telegramBot.bot.on(message('text'), async (ctx) => {
      try {
        ...
        if (!ctx.session) {
          ...
          ctx.session.messages.push({ role: openAi.roles.SYSTEM, content: psyConfig });
        }
        ...
        if (JSON.stringify(telegramBot.whitelist.users).includes(ctx.message.from.username)) {
          ...
          ctx.session.messages.push({ role: gptResponse.role, content: gptResponse.content });
    ...
    try {
      if (config.get('SAVE_CHAT_HISTORY')) {
        const userData = {
          message: text,
          path:
            config.get('SAVE_VOICE_HISTORY') === 'request' || config.get('SAVE_VOICE_HISTORY') === 'any'
              ? filePath
              : null,
        };
        const assistantData = {
          ...
          path:
            config.get('SAVE_VOICE_HISTORY') === 'response' || config.get('SAVE_VOICE_HISTORY') === 'any'
              ? responseVoicePath
              : null,
        };
